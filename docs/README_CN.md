...
- [x] 测试套件 — 将测试用例组合为自动化序列（Agent 级 + 工作空间级）
- [x] 断言验证 — 7 种类型：状态匹配、JSONPath 等值/存在/包含/正则、响应时间、媒体检测
- [x] CI 集成 — 命令行运行测试套件
- [x] 测试报告 — 导出 HTML/JSON 格式报告

### v0.3 — 本地注册代理 ✅

- [x] 本地 A2A 注册中心 — 内嵌 axum HTTP 代理，聚合 Agent 发现
- [x] 请求/响应拦截与修改
- [x] 延迟模拟与故障注入
- [x] 流量录制与回放
...
- [x] 社区 Agent 目录 — 发现和加载优质 A2A Agent
- [x] 共享测试集 — 导入/导出社区测试套件
- [x] Agent 健康监控 — 定期刷新卡片并告警
- [x] 收藏夹 — 收藏和整理常用 Agent

### v0.5 — 高级工作空间 ✅

- [x] 工作空间共享 — 导出/导入完整工作空间配置
- [x] 环境变量 — 按工作空间的变量替换，支持密钥脱敏
- [x] 请求链 — 将一个技能的输出通过 JSONPath 提取传递给另一个
- [x] Diff 视图 — 跨运行对比响应结果
