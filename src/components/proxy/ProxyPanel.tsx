    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{
          ...
          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
            {status.running ? (
              <span>
                ...
                {t("proxy.runningOn")} :{status.port}
              </span>
            ) : (
              <span style={{ color: "var(--text-muted)" }}>{t("proxy.stopped")}</span>
            )}
          </div>
        </div>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          >
            ...
            {status.running ? t("action.stop") : t("action.start")}
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "0.5px solid var(--border-subtle)", flexShrink: 0 }}>
        {(["rules", "recording", "traffic"] as ProxyTab[]).map((tab) => (
          ...
          >
            {tab === "rules" ? t("proxy.rules") : tab === "recording" ? t("proxy.recording") : t("proxy.traffic")}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        ...
        {activeTab === "recording" && <RecordingTab workspaceId={activeWorkspaceId} recordings={recordings} isRecording={isRecording} proxyRunning={status.running} />}
  ...
  return (
    <div>
      <div style={{ padding: "8px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        }}>{t("proxy.newRule")}</button>
      </div>

      {showCreate && (
        <div style={{ padding: "8px 12px", borderBottom: "0.5px solid var(--border-subtle)", display: "flex", flexDirection: "column", gap: 4 }}>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t("proxy.ruleName")} style={inputStyle} autoFocus />
          <div style={{ display: "flex", gap: 4 }}>
            <select value={matchType} onChange={(e) => setMatchType(e.target.value)} style={{ ...inputStyle, flex: 1 }}>
              <option value="all">{t("proxy.matchAll")}</option>
              <option value="agent">{t("proxy.matchAgent")}</option>
              <option value="skill">{t("proxy.matchSkill")}</option>
              <option value="method">{t("proxy.matchMethod")}</option>
            </select>
            {matchType !== "all" && (
              <input value={matchValue} onChange={(e) => setMatchValue(e.target.value)} placeholder={t("input.headerValue")} style={{ ...inputStyle, flex: 1 }} />
            )}
          </div>
          <div style={{ display: "flex", gap: 4 }}>
            <select value={actionType} onChange={(e) => setActionType(e.target.value)} style={{ ...inputStyle, flex: 1 }}>
              <option value="delay">{t("proxy.actionDelay")}</option>
              <option value="error">{t("proxy.actionError")}</option>
              <option value="mock">{t("proxy.actionMock")}</option>
              <option value="modify_request">{t("proxy.actionModifyReq")}</option>
              <option value="modify_response">{t("proxy.actionModifyResp")}</option>
            </select>
          </div>
          ...
          <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
            <button onClick={() => setShowCreate(false)} style={{ padding: "3px 10px", fontSize: 11, background: "transparent", border: "0.5px solid var(--border-subtle)", borderRadius: "var(--radius-md, 6px)", color: "var(--text-secondary)", cursor: "pointer" }}>{t("action.cancel")}</button>
            <button onClick={handleCreate} style={{ padding: "3px 10px", fontSize: 11, background: "transparent", border: "0.5px solid var(--border-strong)", borderRadius: "var(--radius-md, 6px)", color: "var(--text-primary)", cursor: "pointer", fontWeight: 500 }}>{t("proxy.create")}</button>
          </div>
        </div>
      )}
        ...
        <div key={rule.id} style={{
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>
              {rule.matchType === "all" ? t("proxy.allRequests") : `${rule.matchType}: ${rule.matchValue}`} → {rule.actionType}
            </div>
          </div>
          <div style={{ display: "flex", gap: 4, alignItems: "center", flexShrink: 0 }}>
            ...
            >
              {rule.enabled ? t("proxy.disable") : t("proxy.enable")}
            </button>
            <button
...
// --- Recording Tab ---
function RecordingTab({ workspaceId, recordings, isRecording, proxyRunning }: { workspaceId: string; recordings: import("../../lib/proxy-commands").RecordingSession[]; isRecording: boolean; proxyRunning: boolean }) {
  const { t } = useT();
      ...
      <div style={{ padding: "8px 12px", borderBottom: "0.5px solid var(--border-subtle)" }}>
        {isRecording ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--text-primary)" }}>{t("proxy.recordingActive")}</span>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            <input
              ...
              placeholder={t("proxy.sessionName")}
              style={{
            ...
            />
            <button onClick={handleStart} disabled={!sessionName.trim() || !proxyRunning} style={{
              padding: "3px 10px", fontSize: 11, background: "transparent",
              border: "0.5px solid var(--border-strong)", borderRadius: "var(--radius-md, 6px)",
              color: "var(--text-primary)", cursor: sessionName.trim() && proxyRunning ? "pointer" : "default",
              fontWeight: 500, opacity: sessionName.trim() && proxyRunning ? 1 : 0.5,
            }}>{t("proxy.record")}</button>
          </div>
        )}
      </div>
        ...
        <div key={rec.id} style={{ borderBottom: "0.5px solid var(--border-subtle)" }}>
          ...
          {expandedId === rec.id && (
            <div style={{ padding: "4px 12px 8px", fontSize: 11, fontFamily: "var(--font-mono, monospace)" }}>
              <div style={{ color: "var(--text-muted)", marginBottom: 4 }}>{t("proxy.request")}</div>
              ...
              {rec.responseJson && (
                <>
                  <div style={{ color: "var(--text-muted)", marginTop: 8, marginBottom: 4 }}>{t("proxy.response")}</div>
                  <pre style={{ margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all", color: "var(--text-secondary)", maxHeight: 150, overflow: "auto" }}>
